...
const AgeDateSelector: React.FC<AgeDateSelectorProps> = ({ onAgeSelect, onSelection, onContinue }) => {
    ...
    // Fetch existing date of birth on component mount
    useEffect(() => {
        let cancelled = false;
        ...
        const fetchDob = async () => {
            try {
                setLoading(true);
                const res = await api.get("/questionnaire/date-of-birth");
                if (cancelled) return;

                const dobString = res?.data?.data?.dateOfBirth;
                if (dobString) {
                    const dob = new Date(dobString);
                    if (!isNaN(dob.getTime())) {
                        const dobMonth = months[dob.getMonth()];
                        ...
                        const calcAge = calculateAge(dobMonth, dobDay, dobYear);
                        setAge(calcAge);
                        if (onAgeSelect) onAgeSelect(calcAge);
                        if (onSelection) onSelection(calcAge, dobYear);
                    }
                }
            } catch (error) {
    ...
    const handleContinue = async () => {
        try {
            ...
            // Call the onContinue callback after successful save
            if (onContinue) {
                onContinue(age, selectedDate.year, selectedDate.month, selectedDate.day);
            }
        } catch (error) {
            console.error('Error saving date of birth:', error);
            // You can add proper error handling here (toast, alert, etc.)
        } finally {
            setLoading(false);
        }
    };
    ...
    const DateColumn = ({
        ...
        return (
            <div className="flex flex-col items-center">
                ...
                <div
                    ref={containerRef}
                    className={`flex flex-col items-center space-y-2 select-none cursor-grab touch-none transition-transform duration-200 ${isDragging ? 'cursor-grabbing' : ''
                        }`}
                    style={{
