...
import type { Achievement, Challenge, Plan, UserProfile, UserPreferences } from "@/types/user.types";
import { useAuthStore, useUserStore, useFitnessStore, useUIStore } from "@/stores";
...
const Dashboard = () => {
  ...
  const { activePlans, availableChallenge, fetchActivePlans, fetchAvailableChallenges, isLoading: fitnessLoading } = useFitnessStore();
  const { sidebarOpen, setSidebarOpen } = useUIStore() as {
    sidebarOpen: boolean;
  ...
  // Use profile from store, fallback to user from auth store
  const currentUser = (profile || user) as UserProfile;
  const loading = userLoading || fitnessLoading;

  // Use challenges from fitness store
  const challenges = availableChallenge;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // TODO: Fetch achievements when API is available
        // setAchievements(...);
      } catch (err) {
    ...
    fetchData();
  }, [fetchProfile, fetchActivePlans, fetchAvailableChallenges]);
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600">No user data available. Please try logging in again.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center">
              <div className="flex items-center">
                <img
                  ...
                  src={currentUser.profileImage || "https://placehold.co/400x400"}
                  alt="Profile"
                />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex flex-col md:flex-row gap-6">
          {/* Sidebar */}
          <div
            className={`fixed rounded-xl inset-y-0 left-0 z-20 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
              }`}
          >
              ...
              <div className="p-6 border-b border-gray-200">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-gray-500 text-sm">
                    @{currentUser.firstName || "user"}_{currentUser.id ? currentUser.id.slice(0, 6) : ""}
                  </p>
                  ...
                  <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 hover:shadow-md transition-shadow">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-2xl font-bold text-gray-900">
                          {getCaloriesBurned(currentUser.activityLevel)}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                    ...
                    <div className="mt-2 md:mt-0">
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${bmiCategory === "Underweight"
                  ...
                  <div className="border-t border-gray-200">
                    <dl>
                      ...
                      <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                          {currentUser.firstName ? `${currentUser.firstName}_${currentUser.id ? currentUser.id.slice(0, 6) : ""}` : "Not set"}
                        </dd>
                      </div>
                      <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt className="text-sm font-medium text-gray-500">
                      ...
                      <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                          {currentUser.activityLevel ? currentUser.activityLevel.replace(/_/g, " ") : "Not specified"}
                        </dd>
                      </div>
                      <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt className="text-sm font-medium text-gray-500">
                    ...
                    >
                      <div className="p-6">
                        ...
                        <div className="mt-6 flex space-x-3">
                          <button
                            className={`flex-1 inline-flex justify-center items-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white ${challenge.completed
                  ...
                  <div className="border-t border-gray-200">
                    <dl>
                      ...
                      <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${preferences.notificationsEnabled
                              ? "bg-green-100 text-green-800"
                              : "bg-red-100 text-red-800"
                              }`}
                          >
